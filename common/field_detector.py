...
class FieldDetector:
    ...
    def detect_field(self, field_type: str, record: Dict = None) -> Optional[str]:
        ...
        import os
        env_mappings = {
            'participant_email': os.getenv('EMAIL_FIELD_NAME'),
            'qids_score': os.getenv('QIDS_FIELD_NAME'),
            'eligibility': os.getenv('ELIGIBILITY_FIELD_NAME'),
            'study_id': os.getenv('STUDY_ID_FIELD_NAME')
        }
        
        if field_type in env_mappings and env_mappings[field_type]:
            self._field_cache[field_type] = env_mappings[field_type]
            return env_mappings[field_type]
